...
void input(){
    ...
    for(int i = 1; i<=m; i++){
        cin >> u >> v >> w;
        a[u].push_back(uu(w, v)); // (trong so, dinh)
        a[v].push_back(uu(w, u)); // (trong so, dinh)
    }
    cin >> k;
    for(int i = 0; i<k; i++){
        cin >> u;
...
int dd(int ss){ // tim duong di dai nhat tu s toi cac dinh con lai trong S
    //khoi tao:
    for(int i = 1; i<=n; i++){
        if(i != ss){
            d[i] = MAX;
...
//    }
    ...
    dmax= -10000; // tim khoang cach ngan nhat giua s voi cac dinh con lai trong S
    for(int i = 0; i<k; i++){
        int v = vS[i];
        if(v != ss && d[v] != MAX && d[v] > dmax) dmax = d[v];
    }
    return dmax;
}
